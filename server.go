...
// reviewPasses is shared by both review prompts: on top of the reviewers' output,
// a structured five-pass sweep over the FINAL whole-branch diff (not incremental),
// with findings reported per pass.
const reviewPasses = "On top of the reviewers' output, review the FINAL diff in one shot — the whole-branch diff described above, not incrementally — and run these five passes, reporting findings per pass. For each finding give file:line, what's wrong, and the minimal fix. " +
	"1. DOC-TRUTH PASS: Read every comment, KDoc, log string, and every touched ADR/doc line. For each sentence, assert it is literally true of the FINAL code. Flag anything that describes an earlier version of the code or the change's history. " +
	"2. STATE-MACHINE PASS: Enumerate every status/state a record can hold. For each failure point in the flow, state which status the record ends in. Verify the code comment, the log line, the ADR, and at least one test all agree on that terminal status. Flag any disagreement or missing assertion. " +
	"3. COLD-READER PASS: Judge each comment/KDoc as someone who has never seen this PR or its diff. Flag any that only make sense with PR context: \"extracted from\", \"now\", \"previously\", \"first cut\", \"the caller re-validates\", \"this used to\". Comments must describe the code as it is, not how it got there. " +
	"4. CONCURRENCY PASS: For every read that feeds a write decision, name the lock/transaction it runs under and whether the decision needs the value fresh. Flag reads done outside the lock that a later write depends on (TOCTOU). For batch operations, state whether items are claimed individually or as a batch, and why that's safe. " +
	"5. NECESSITY + NAMING PASS: For each new construct (metric, status value, service, Provider injection, DTO, public method), state why it must exist and what breaks without it; if the same info exists elsewhere (e.g. a history table), justify the duplication. For each new public name, check it reads correctly from the caller's side."
